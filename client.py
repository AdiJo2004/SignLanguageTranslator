...
try:
    ...
    time.sleep(2)
    #Note the start time and construct a stream to hold image data temporarily(we could write it directly to connection
    #but in this case we want to find out the size of each capture first to keep out protocol simple)
    start=time.time()
    stream-io.BytesIO()
